...
//    a[2i+1]  a[2i+2]
```
而container/heap包采用的是a[0]作为堆顶，故左子、右子节点以及父节点表示为a[2i+1]、a[2i+2]、a[(i-1)/2]。
...
由以上源码可知，要实现一个heap数据结构，必须实现Len()、Less()（根据这里控制大顶堆还是小顶堆）、Swap()、Push()、Pop()五个方法；`Fix()` 和 `Update()` 是**可选**的便捷方法——当堆内某个元素的值发生变化后，用它们将该元素在堆中**上浮或下沉**到正确位置，而不用先 Remove 再 Push。
...
func (h DataHeap) Less(i, j int) bool { return h[i].weight < h[j].weight }
