...
正如上面说的，goroutine是独立地，可能并行地执行任务。channel是服务于goroutine之间的通信，同步等。
...
ch := make(chan int) //创建一个无缓冲的int型的channel，无缓冲channel的发送会一直阻塞，直到有接收方准备好并完成接收

ch := make(chan int, 10) //创建一个缓冲区大小为10的int型的channel

ch <- x //将x发送到channel中，如果channel缓冲区满（无缓冲则始终视为满），则阻塞当前goroutine直到有接收方

<- ch //从channel中接收一个值，如果缓冲区为空，则阻塞直到有数据

x = <- ch //从channel中接收一个值并存到x中，如果缓冲区为空，则阻塞直到有数据

x, ok = <- ch //从channel中接收一个值，如果channel已关闭，则ok为false；channel未关闭且为空时，会阻塞直到有数据
...
有缓存的channel的构造方式为指定make函数的第二个参数，该参数为channel缓存的容量。
```go
ch := make(chan int, 10)
```
...
type hchan struct {
    ...
    recvq    waitq  // list of recv waiters  ： recv的goroutine的等待队列
    sendq    waitq  // list of send waiters  ： send的goroutine的等待队列
...
我们通过上面的图来表示发送和接收的流程，刚开始的时候，缓冲区都是空的。如果开始发送，那么变成下面这种状态（原配图已丢失），并且sendx开始累加。


缓冲区发送满了以后（sendx 追平 recvx）就会阻塞后续的发送方。
...
- 获取同步锁，对缓冲队列进行加锁
- 如果有等待的goroutine，那么数据直接发送给这个接收goroutine，数据拷贝1次
- 如果缓冲队列没有满，将发送的数据拷贝到队列中，sendx加1
- 如果缓冲队列满了，那么将发送goroutine放到发送阻塞队列中

#### channel的recv方法

在发送的章节，我们已经把缓冲区里面存放了数据，然后就是接收的goroutine开始接收数据了。
...
如果这时候recv的goroutine开始接收数据，那么找到被阻塞的send goroutine的sudog， 然后把sudog的队列的数据塞到缓冲区队列里面，并且激活这个sudog对象，让里面的goroutine成为runnable。

总结

回到刚开始的那句话（原配图已丢失，含义是：channel + goroutine 通过阻塞/唤醒实现了同步与通信）：
