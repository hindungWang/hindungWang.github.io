...
需要说明的是，这是 Prometheus 的**设计选择**（单机 TSDB + 生态扩展），而非缺陷：官方不做内置集群，高可用靠部署模式（双实例 + Query 去重），扩展靠分片（hashmod 抓取分流）或外部组件（Thanos / Mimir / VictoriaMetrics）。
...
这里需要说明瓶颈归属：这是 **prometheus-operator 的 config-reloader 侧瓶颈**（sd 过多 → ConfigMap 过大 → reload 慢/超时 → 重启循环），而不是 Prometheus 本身——在评估监控系统负载时，要把采集端与配置下发组件区分开看。
...
需要说明测试方法：本组 target 测试固定了每 target 的 series 数，真正该关注的量纲是"**总 series 数 × 抓取频率**"，target 数本身的开销远小于其承载的 series，因此结论能把 target 数与 series 总量解耦。
...
需要说明测试环境边界：500w series 时节点挂掉，受限于测试环境 **8C/24G + 机械盘**（写速率 ~10MB/s、IOPS ~260）——500w series 时 WAL 写放大极大，瓶颈很可能是磁盘 IOPS，而不完全是 Prometheus 内存。在内存充足（active series × ~1-3KB/series）+ SSD 的场景下，单机可支撑更高的量级，因此该结论仅在上述环境边界内成立。

这一结果也与理论自洽：100w series、30s 抓取、2h 一个 block = 100w × 240 samples ≈ 2.4 亿样本，gorilla 压缩后平均 ~1-2 字节/sample，约 240-480MB/block，与上文实测单块 ~400MB 吻合。
...
需要说明演练边界：本次测试只验证了"down 节点监控不断"，且**未配置 Receiver hashring 副本（replication factor）**——如果只做数据打散而没有副本，挂掉一个 Receiver 节点时，该段数据在恢复前同样不完整。生产环境要做到真正的 HA，需要配置 replication factor，并补上"节点恢复后数据补齐"的观测（WAL 回放 → 数据补全），才算完整闭环。

历史数据出现缺失是因为挂掉的节点正好存储了那一部分的数据，没来得及存储到minio，但是不影响最新的数据，等节点启动会恢复该段数据。

这里补充一下数据缺失的机制：节点 down 时，WAL 里**尚未固化成 block**（最近约 2h）的数据来不及上传到对象存储，因此这部分历史会暂时缺失。Compactor 的职责是降采样（downsampling）、合并小 block、清理 tombstone、按 retention 删过期数据——它**不负责补数/恢复数据**。节点只是重启时，WAL 会在启动时回放，数据自动恢复（Prometheus 崩溃恢复机制）；若节点/磁盘彻底丢失，最近 2h 数据确实会丢，需要通过 **Receiver 多副本**（remote-write 实时推送 + hashring replication factor）或**双实例采集 + Query 去重**来兜底。对告警实时性场景而言，告警评估用的是实时数据，丢失"最近 2h 历史"通常可以接受。

在 Receiver 模式下，采集端 Prometheus **仍然要写本地 WAL**（先写本地再 remote-write），并非真正无状态；本地崩溃时只丢几秒（remote-write buffer），因为数据已实时推到 Receiver/对象存储。但 **Receiver 本身是有状态的**——hashring 上某节点挂了，该段数据在恢复前不完整，除非配置了 replication factor（副本）。真正"轻量无状态"的是 push-only 场景（avalanche 这类直接 remote-write，或 Prometheus agent mode）。

*但是使用Receiver模式占用的资源很大，组件多，从监控上看整个HA大概需要30+GiB内存和3+Core CPU*

这些额外开销来自 remote-write 接收、hashring 分发和更大的 WAL。两种模式的选型不是"谁更好"，而是取决于网络拓扑：**Receiver 适合"采集端无法直连对象存储 / 边缘跨网络"的场景**（用副本因子换取更小的丢失窗口）；**Sidecar 适合"Prometheus 能直连对象存储"的场景**（组件更少、开销更低）。
...
这里的测试条件是**近期数据、低并发、近 24h 范围**内的查询；Thanos Query 查询对象存储中的老数据（StoreAPI）会明显变慢，历史深查询需要加 Query response cache，必要时配合 downsampling。
...
具体来说，Gossip（memberlist）在这里是**同步机制**，去重是它带来的**结果**：Alertmanager 集群通过 Gossip 协议同步"谁已通知过"的状态和静默（silences），从而实现**同一告警只通知一次（去重）**、多实例静默一致以及 flapping 抑制。
...
需要说明：这里压的是 **PrometheusRule 规则数量**（1w/3w/10w/20w 条规则），而不是活跃告警数。Prometheus 默认每 15s 评估一次所有规则，**每条规则 ≈ 一次查询**，20w 条规则 = 每 15s 执行 20w 次查询 → CPU 打满。因此**规则数量必须受控**——20w 规则本身就是极端不合理的配置（正常集群只有几十到几百条），实战应使用 recording rules 预聚合、按需调整 evaluation_interval，并按集群级/应用级对规则分层。
