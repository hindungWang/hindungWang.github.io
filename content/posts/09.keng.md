...
此外，如果请求的body没有完全读出，将会导致tcp连接不能复用。
...
func main(){
	...
	for i := 0; i < 10; i++{
		//fmt.Printf("i=%d\n", i)
		b := []byte("id2")
		// 坑：b 是局部变量，每次循环复用同一块底层数组。
		// 字符串转换 *(*string)(unsafe.Pointer(&b)) 让 string 的 data 指针直接指向 b 的底层数组，
		// 而 b 的内容在下一轮循环被覆盖成 "id1"，于是 map 里存的 key 引用着同一块被反复改写的内存，
		// 最终所有 key 的值都会变成最后一次写入的 "id1"（map 计算 hash 用字面值 "id2"，取 key 却是 "id1"）。
		// 正确做法：每次循环用 string(b) 拷贝一份独立的字符串。
