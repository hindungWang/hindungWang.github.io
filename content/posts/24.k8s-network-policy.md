...
默认情况下，规则的生效是`只允许`策略，规则声明则允许，规则外的默认阻塞。需要注意：上面 `from` 列表中的 `namespaceSelector`、`podSelector`、`ipBlock` 三项是**「或（OR）」关系**——只要命中其中任意一项的流量就会被放行，而不是同时满足。若希望多个条件同时满足，需把它们放在同一个 `podSelector`/`namespaceSelector` 项内或用多条规则叠加。
