...
## 分析
从监控上看，异常节点的整体 CPU 使用率已占满（图中 CPU% 接近 100%），其中 k3s-server 进程的 CPU 占用超过 200%（即超过 2 个核），top 里 k3s-server 的 CPU 占比也明显高于正常节点，因此开始怀疑是不是底层虚拟化有 CPU 分配的问题。
...
然而，占用最多的居然是系统调用中的 GC 方法。此外没有过多信息。

又请教虚拟化相关的同事，对虚拟机配置一顿操作（绑定 numa，分配 IO thread，限制 CPU 超分 等），然而还是没能解决。
