...
NAT 包括 SNAT （源地址转换）和 DNAT （目的地址转换）。SNAT 修改报文的源地址，作用于 POSTROUTING 链（路由后、发出前）；DNAT 修改报文的目的地址，作用于 PREROUTING 链（路由前）。两者通常配合使用以实现双向通信，但**并非总是成对出现**——例如仅做内网出口上网时只需 SNAT，仅做端口映射时只需 DNAT。
