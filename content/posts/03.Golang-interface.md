...
这种实现是`DuckType`：类型不需要显式声明它实现了某个接口：接口被隐式地实现。多个类型可以实现同一个接口。
...
任何`interface`类型，在内存中都是2个机器字长（word），32位机器上是8`Byte`，64位机器上是16`Byte`。
...
对于接口的`type-switch`（如 `v := x.(type)`），`v` 得到的和反射里 `reflect.TypeOf` 返回的都是**动态类型**（即接口里实际存储的具体类型），二者本质相同。接口被赋值为某个具体类型的值后，其内部保存的是该具体类型的类型信息，因此静态声明的接口类型与运行时的动态类型要区分开。
...
+========+            | type   |
                      ...
                      |   ...  |
```
所以这个时候把一个其他类型的`nil`指针赋值给`interface{}`的时候，`interface`并不是空，而是对`interface`进行了初始化，只不过`data`是`nil`而已。所以如果函数返回的是`interface{}`或者其他有方法的`interface`（比如`error`），就需要注意：不要直接返回某个类型的空指针，而应该显式返回`nil`。如：
```Go
