...
|    ENTRYPOINT     |   容器入口点，不可被`docker run`覆盖  	    |   `ENTRYPOINT` [“executable”, “param1”, “param2”]或`ENTRYPOINT command param1 param2` |   `ENTRYPOINT entrypoint.sh`    |
...
```
此外，如果在`ADD`，`COPY`指令中添加了多余的文件，此后在其他层删除掉该文件或文件夹虽然文件删除了，但是镜像大小是不变或增加的：
...
就像某些程序，编译出来的可执行文件只有十几`kb`，其他的都是以动态库的形式存在，那么怎么寻找应用对应的依赖库呢？
...
|   Fedora    |  33  |    67.21   |   稳定版本    |    dnf/yum，rpm   |
