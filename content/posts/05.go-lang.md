...
- 基本类型（int, string, uint, int8, int16, byte, rune, uintptr, float32, float64, complex64, complex128, bool等）
- 数组和slice的内存模型（string是特殊的slice）
- 指针的内存模型及uintptr与unsafe.Pointer、普通指针 *T的区别
- map的内存模型
- struct的内存模型和字节对齐
...
切片是对数组中一段数据的引用。在内存中它由三段数据组成：一个指向数据头的指针、切片的长度、切片的容量。长度是索引操作的上界，如：x[i] 。容量是切片操作的上界，如：x[i:j]。
...
从上面的内存模型来看，如果两个数组相互赋值，那么将会触发数组全量拷贝的动作，但是如果是传递切片，那么只需要复制固定大小的切片头就可以了，底层的数组通过引用传递。
...
在64位系统上面，Go语言的结构体默认按8字节对齐，如果不足的，就补充padding。
